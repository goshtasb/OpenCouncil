...
export function registerExecutionCommands(program: Command, { repoRoot, config }: CliContext): void {
  program
    ...
    .action(action(async (sessionId: string) => {
      const worktree = await new ExecutionEngine(config).handoff(sessionId);
  ...
  program
    ...
    .action(action(async (sessionId: string, options: { skipAgent?: boolean }) => {
      const outcome = await new ExecutionEngine(config).run(sessionId, { skipAgent: options.skipAgent });
