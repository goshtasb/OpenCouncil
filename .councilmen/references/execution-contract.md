...
8. Work only on the current branch in your isolated clone. Commit all of your changes; leave no uncommitted files. Do not push and do not open a pull request — the harness runs the project's verification gates (lint, tests, and any dependency, secret-scan, static-analysis, accessibility or performance gates the project configures), then pushes and opens the pull request. If a required gate fails, you are resumed with its output. Run those same commands yourself before declaring DONE.
